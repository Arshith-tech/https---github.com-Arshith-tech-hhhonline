...
// Memoized Dropdown component for desktop navigation
const Dropdown = ({ items }: { items: typeof navigation[number]['items'] }) => (
  <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-lg shadow-lg border pointer-events-auto">
    <div className="py-2">
      {items.map((subItem) => (
        <Link
          key={subItem.name}
...
const Header = () => {
  ...
  // Debounced handlers to reduce flickering on hover
  ...
  const handleMouseLeave = () => {
    ...
    }, 200); // Delay closing dropdown by 200ms
  };
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navigation.map((item) => (
              ...
              >
                ...
                {activeDropdown === item.name && <Dropdown items={item.items} />}
              </div>
            ))}
